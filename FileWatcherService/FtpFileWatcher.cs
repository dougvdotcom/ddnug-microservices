...
namespace FileWatcherService
{
    internal class FtpFileWatcher
    {
        ...
        private const string FtpPath = @"c:\inetpub\ftproot\ftpuser";
        private const string SqlConn =
                "Server=tcp:ddnug.database.windows.net,1433;Initial Catalog=ddnugdb;Persist Security Info=False;User ID=pinehead;Password=LinuxAcademy1;MultipleActiveResultSets=False;Encrypt=True;TrustServerCertificate=False;Connection Timeout=30;"
            ;
        private const string StorageConn =
                "DefaultEndpointsProtocol=https;AccountName=ddnugfunctionstorage;AccountKey=xprd9hPQMkj/gz1aJBUnj47j3Jvq5iAF/09JOsVhJZY+lSMyNNpkcm/Vra6UwTuYhdDMOIYvg1LIWOtwVNlAsw==;EndpointSuffix=core.windows.net"
            ;
        private const string QueueConn =
                "Endpoint=sb://ddnugsb.servicebus.windows.net/;SharedAccessKeyName=RootManageSharedAccessKey;SharedAccessKey=C9YEXasCAiIAbZ3n42LgDjUKf/oUkkU5b3WtfyN0UEA="
            ;
                ...
                try
                {
                    using (var conn = new SqlConnection(SqlConn))
                    {
                        using (var cmd = new SqlCommand("sp_uploaded_create", conn))
                        {
                            ...
                            cmd.Parameters.Add(new SqlParameter("PhotoName", SqlDbType.NVarChar, 50)).Value = imgName;
                            ...
                            cmd.Parameters.Add(new SqlParameter("UploadTime", SqlDbType.DateTime)).Value =
                                DateTime.UtcNow;
                            cmd.Parameters.Add(new SqlParameter("Caption", SqlDbType.NVarChar, 500)).Value =
                                imgCaption;
